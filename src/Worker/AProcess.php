...
final class AProcess extends AbstractWorker
{
        ...
        if ($connected) {
            try {
                ...
                foreach ($work as $_ => $payload) {
                    /**
                     * Whatever happens, always report successful processing
                     */
                    $processed = true;

                    $this->manageForks($forks);
                    ...
                    try {
                        ...
                        if ($deadline > 0 && $deadline < time()) {
                             ...
                             */
                            $this->logDebug('skipping task beyond deadline');
                            $work->send($processed);
                        ...
                        $run = function () use ($message) {
                            $this->logDebug('launching ' . var_export($message->getCommandline(), true));
    ...
    /**
     * Reap finished forked processes, preventing zombies
     *
     * @param array<int, Process> $forks
     */
    private function manageForks(array $forks): void
    {
        foreach ($forks as $f) {
            try {
                /**
